    ...
    return (
        <nav className="flex justify-between items-center p-4 bg-slate-800 text-white h-20 z-50">
            ...
            <ul className="flex space-x-4 gap-40">
                <li
                    className={`${currentSection === 'aboutme' ? 'font-bold' : ''} cursor-pointer`}
                    onClick={() => scrollToSection('aboutme')}
                >
                    À propos de moi
                </li>
                <li
                    className={`${currentSection === 'skills' ? 'font-bold' : ''} cursor-pointer`}
                    onClick={() => scrollToSection('skills')}
                >
                    Compétences
                </li>
                <li
                    className={`${currentSection === 'portfolio' ? 'font-bold' : ''} cursor-pointer`}
                    onClick={() => scrollToSection('portfolio')}
                >
                    Portfolio
                </li>
                <li
                    className={`${currentSection === 'contact' ? 'font-bold' : ''} cursor-pointer`}
            ...
            <div className="relative">
                <button 
                    className="px-2 py-1 border rounded flex gap-2 items-center" 
                ...
                {isDropdownOpen && (
                    <div className="absolute right-0 mt-2 bg-slate-800 border rounded shadow-lg z-10">
                        <button className="block px-4 py-2 text-white hover:bg-slate-900" onClick={() => {toggleDropdown(); alert('Switched to French');}}>
                            Fr
                        </button>
                        <button className="block px-4 py-2 text-white hover:bg-slate-900" onClick={() => {toggleDropdown(); alert('Switched to English');}}>
                            En
                        </button>
                    </div>
                )}
